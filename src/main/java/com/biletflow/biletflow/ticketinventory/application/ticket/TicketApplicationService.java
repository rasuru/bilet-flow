...
import com.biletflow.biletflow.ticketinventory.application.common.EntityNotFoundException;
...
public class TicketApplicationService {
    ...
    @Transactional
    public TicketView linkOwner(LinkTicketOwnerCommand command) {
        Objects.requireNonNull(command, "command cannot be null");

        Ticket ticket = requireTicket(command.ticketId());

        String verifiedEmail = verifiedUserEmailPort
            .findVerifiedEmail(command.userId())
            .orElseThrow(() -> new IllegalStateException("User does not have a verified email"));
        ...
        if (!ticket.getAttendeeEmail().equals(normalizedVerifiedEmail)) {
            throw new IllegalStateException("Verified account email does not match ticket attendee email");
        }

        ticket.linkOwner(command.userId());
